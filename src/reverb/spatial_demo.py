...
def demo_spatial_audio():
    ...
    print(
        f"✓ Created mono test signal: {len(signal.data[0])} samples at {signal.sample_rate}Hz"
    )
    ...
    for pos in positions:
        ...
        print(
            f"  ✓ Spatialized sound from {direction}: {spatialized.channels} channels"
        )
    ...
    for vel in velocities:
        spatialized = platform.spatialize_signal(
            signal, source_pos=(1, 0, 0), velocity=vel
        )
    ...
    print(
        f"  ✓ Output: {full_processed.channels} channels, {len(full_processed.data[0])} samples"
    )
    ...
    spatialized_custom = platform.spatialize_signal(
        signal, source_pos=(2, 1, 0.5), listener_pos=(0, 0, 0), velocity=(5, 0, 0)
    )
    ...
    print(
        "The Reverb platform now supports immersive 3D audio with binaural rendering."
    )
