...
class AudioSignal:
    ...
    data: List[
        List[float]
    ]  # List of channel data (e.g., [[left_samples], [right_samples]])
    ...
    def __post_init__(self):
        if len(self.data) != self.channels:
            raise ValueError(
                f"Data has {len(self.data)} channels but channels parameter is {self.channels}"
            )
    ...
    @classmethod
    def create_stereo(
        cls, left_data: List[float], right_data: List[float], sample_rate: int = 44100
    ) -> "AudioSignal":
