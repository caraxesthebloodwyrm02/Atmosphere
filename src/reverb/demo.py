...
class Reverb:
    ...
    def __repr__(self) -> str:
        return (
            f"Reverb(rt60={self.rt60:.2f}s, pre_delay={self.pre_delay:.3f}s, "
...
def describe_reverb(r: Reverb) -> str:
    ...
    rt60_desc = "short" if r.rt60 < 0.8 else "medium" if r.rt60 < 2.0 else "long"
    pre_delay_desc = (
        "minimal"
        if r.pre_delay < 0.02
        else "moderate" if r.pre_delay < 0.08 else "extended"
    )
    damping_desc = (
        "bright" if r.damping < 0.3 else "balanced" if r.damping < 0.7 else "muffled"
    )
    mix_desc = "dry-only" if r.mix == 0 else "wet-only" if r.mix == 1 else "balanced"
