...
from ..models.signal import AudioSignal, ReverbParameters, SpatialParameters
...
class SpatialAudioService:
    ...
    def _generate_simple_hrtf(self) -> Dict[str, Dict[str, np.ndarray]]:
        ...
        for azimuth_deg in range(0, 360, 45):
            ...
            hrtf_data[str(azimuth_deg)] = {"left": hrtf_left, "right": hrtf_right}

        return hrtf_data

    def _get_hrtf_filters(
        ...
        closest_angle = min(
            self.hrtf_data.keys(),
            key=lambda k: min(
                abs(int(k) - azimuth_deg), 360 - abs(int(k) - azimuth_deg)
            ),
        )
    ...
    def _calculate_doppler_shift(
        ...
        if abs(relative_velocity) < params.speed_of_sound:
            doppler_factor = params.speed_of_sound / (
                params.speed_of_sound - relative_velocity
            )
        else:
    ...
    def process(
        ...
        left_ear, right_ear = self.spatialize_sound(
            mono_data, source_pos, listener_pos, velocity
        )
