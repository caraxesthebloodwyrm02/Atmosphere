...
def visualize_network(network, save_path: Optional[str] = None) -> Optional[BytesIO]:
    ...
    for node in network.graph.nodes():
        ...
        for neighbor in network.graph.neighbors(node):
            edge_key = (
                (node, neighbor)
                if (node, neighbor) in network.acoustic_params
                else (neighbor, node)
            )
        ...
        if connected_params:
            ...
            depth_factor = (
                avg_density * 0.6 + (1 - avg_delay / 10000) * 0.3 + avg_feedback * 0.1
            )
    ...
    for u, v in network.graph.edges():
        if (u, v) in network.acoustic_params:
            ...
            delay_norm = (
                (params.delay_time - delay_min) / (delay_max - delay_min)
                if delay_max > delay_min
                else 0.5
            )
            feedback_norm = (
                (params.feedback - feedback_min) / (feedback_max - feedback_min)
                if feedback_max > feedback_min
                else 0.5
            )
            density_norm = (
                (params.reverb_density - density_min) / (density_max - density_min)
    ...
    for node in network.graph.nodes():
        x, y = pos[node]
        shadow_offset = (
            1 - node_depths.get(node, 0.5)
        ) * 3  # Further back = larger shadow
