    ...
    def _dispatch_to_zone(self, tool_name, file_path, zone_name):
        ...
        try:
            ...
            if file_path.suffix == ".json":
                # JSON payload - route to network node processor
                processor_path = self.zones_root.parent / "tools" / "network_node_processor.py"
                if processor_path.exists():
                    logger.info(f"Processing JSON payload with network_node_processor")
                    proc = subprocess.Popen(
                        [sys.executable, str(processor_path), str(file_path), str(zone_dir)],
                        stdout=open(log_file, "w"),
                        stderr=subprocess.STDOUT,
                        cwd=str(zone_dir)
                    )
                else:
                    logger.warning(f"Network node processor not found at {processor_path}")
                    # Just log the payload
                    import json
                    with open(file_path, 'r') as f:
                        payload = json.load(f)
                    with open(log_file, "w") as lf:
                        lf.write(f"[JSON Payload] Tool: {tool_name}\n")
                        lf.write(f"[JSON Payload] Zone: {zone_name}\n")
                        lf.write(f"[JSON Payload] Content:\n{json.dumps(payload, indent=2)}\n")
                    # Mark as processed
