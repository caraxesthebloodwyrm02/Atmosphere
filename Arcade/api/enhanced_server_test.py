...
class SystemMonitorPlugin(CommandPlugin):
    ...
    def system_info(self, session):
        ...
        try:
            ...
            # Disk info - use platform-aware root path
...
class EnhancedTerminalHandler:
    ...
    async def process_command(self, session_id: str, command: str):
        ...
        # Try local natural language processing first
        try:
            local_nlp_result = await self._process_local_nlp(session, command)
            if local_nlp_result is not None:
                return local_nlp_result
        except Exception as e:
            logger.warning(f"Local NLP processing failed: {e}")

        # Check if this is a system information command - SKIP AI entirely
        command_lower = command.lower().strip()
        system_keywords = [
            'system', 'computer', 'machine', 'cpu', 'processor', 'memory', 'ram',
            'disk', 'storage', 'drive', 'network', 'uptime', 'processes', 'task'
        ]
        is_system_command = any(keyword in command_lower for keyword in system_keywords)

        if is_system_command:
            # For system commands that don't match our patterns, return error instead of AI
            return f"❌ Unrecognized system command: '{command}'\nTry: 'sysinfo', 'cpu', 'memory', 'disk', 'processes', 'network', or 'uptime'"

        # Use AI interpretation for other natural language (non-system commands)
        try:
            enhanced_context = f"""
...
Just type naturally in ANY language - the AI understands you! 🌍🤖"""
    ...
    async def _process_local_nlp(self, session, command: str):
        ...
        # System information commands - expanded patterns (NO AI FALLBACK)
        elif any(phrase in command_lower for phrase in [
            'system information', 'system info', 'computer info', 'machine info',
            'show system information', 'show system info', 'show computer info',
            'display system information', 'display system info',
            'get system information', 'get system info',
            'tell me system information', 'tell me system info'
        ]):
            # Call the sysinfo plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
                return self.plugins['system'].system_info(session)
            return "❌ System monitoring plugin not available"

        elif any(phrase in command_lower for phrase in [
            'disk usage', 'disk space', 'storage info', 'disk info',
            'show disk usage', 'show disk space', 'show storage info',
            'display disk usage', 'display disk space',
            'get disk usage', 'get disk space'
        ]):
            # Call the disk usage plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
                return self.plugins['system'].disk_info(session)
            return "❌ System monitoring plugin not available"

        elif any(phrase in command_lower for phrase in [
            'cpu info', 'processor info', 'cpu usage', 'cpu information',
            'show cpu info', 'show processor info', 'show cpu usage',
            'display cpu info', 'display processor info',
            'get cpu info', 'get processor info'
        ]):
            # Call the CPU info plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
                return self.plugins['system'].cpu_info(session)
            return "❌ System monitoring plugin not available"

        elif any(phrase in command_lower for phrase in [
            'memory info', 'ram info', 'memory usage', 'memory information',
            'show memory info', 'show ram info', 'show memory usage',
            'display memory info', 'display ram info',
            'get memory info', 'get ram info'
        ]):
            # Call the memory info plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
                return self.plugins['system'].memory_info(session)
            return "❌ System monitoring plugin not available"

        elif any(phrase in command_lower for phrase in [
            'running processes', 'process list', 'tasks', 'running tasks',
            'show processes', 'show running processes', 'show process list',
            'display processes', 'display running processes',
            'list processes', 'list running processes',
            'get processes', 'get running processes'
        ]):
            # Call the processes plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
                return self.plugins['system'].process_list(session)
            return "❌ System monitoring plugin not available"

        elif any(phrase in command_lower for phrase in [
            'network info', 'network status', 'network information',
            'show network info', 'show network status',
            'display network info', 'display network status',
            'get network info', 'get network status'
        ]):
            # Call the network info plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
                return self.plugins['system'].network_info(session)
            return "❌ System monitoring plugin not available"

        elif any(phrase in command_lower for phrase in [
            'uptime', 'system uptime', 'how long running', 'system running time',
            'show uptime', 'show system uptime',
            'display uptime', 'display system uptime',
            'get uptime', 'get system uptime'
        ]):
            # Call the uptime plugin command - NO AI FALLBACK
            if 'system' in self.plugins:
        ...
        # NO AI FALLBACK FOR UNRECOGNIZED COMMANDS
        return None
